...
class Settings:
    ...
    stop_loss_pct: float = _f("STOP_LOSS_PCT", 0.10)
    smart_stop_loss_enabled: bool = _b("SMART_STOP_LOSS_ENABLED", True)
    stop_loss_partial_pct: float = _f("STOP_LOSS_PARTIAL_PCT", 0.05)
    stop_loss_partial_fraction: float = _f("STOP_LOSS_PARTIAL_FRACTION", 0.50)
    max_hold_seconds: int = _i("MAX_HOLD_SECONDS", 90)
    take_profit_scaleout_pct: float = _f("TAKE_PROFIT_SCALEOUT_PCT", 0.05)
    take_profit_soft_pct: float = _f("TAKE_PROFIT_SOFT_PCT", 0.10)
    take_profit_hard_pct: float = _f("TAKE_PROFIT_HARD_PCT", 0.15)
